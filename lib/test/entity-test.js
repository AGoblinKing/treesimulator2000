  ...
  vows.describe("Entity").addBatch({
    "An Entity": {
      ...
      "has change events": {
        topic: function() {
          ...
          entity.once("change:x", function(_arg) {
            var value;
            value = _arg.value;
            return _this.callback(null, value, entity);
          });
          entity.x = 1;
        },
        "that fire when property is changed": function(err, value, entity) {
          return assert.equal(value, 1);
        },
        "has bindings": function(err, value, entity) {
      ...
      "can instantiate from data": {
        ...
        "has properties set": function(entity) {
          assert.equal(entity.x, 5);
          return assert.equal(entity.potassium, 0);
        },
        "has a goal": function(entity) {
          var _ref;
          assert.isArray(entity.goals);
          return assert.equal((_ref = entity.goals) != null ? _ref.length : void 0, 1);
        }
      },
